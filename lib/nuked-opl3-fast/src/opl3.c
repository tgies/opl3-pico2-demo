 ...
 *     into OPL3_EnvelopeUpdateRate (eg_rate_word[4]).
...
/* OPL3_HOT_IN_RAM places hot functions and tables in RAM; no-ops otherwise. */
#ifdef OPL3_HOT_IN_RAM
#define OPL3_RAM_TABLE __attribute__((section(".data.opl3_table")))
#define OPL3_HOT       __attribute__((section(".time_critical"))) __attribute__((hot))
#define OPL3_FORCE_INLINE __attribute__((always_inline)) inline
#else
#define OPL3_RAM_TABLE
#define OPL3_HOT
#define OPL3_FORCE_INLINE inline
#endif

/* Profiling counters (OPL3_PROFILE builds only). ProcessSlot bumps one tier
 * counter; the FULL path also brackets its four stages with DWT CYCCNT. */
#ifdef OPL3_PROFILE
volatile uint32_t opl3_prof_trivial;
volatile uint32_t opl3_prof_silent;
volatile uint32_t opl3_prof_sustain;
volatile uint32_t opl3_prof_full;
volatile uint32_t opl3_cyc_fb, opl3_cyc_env, opl3_cyc_phase, opl3_cyc_slotgen;
#define OPL3_CYCCNT (*(volatile uint32_t *)0xE0001004u)   /* DWT CYCCNT */
#define OPL3_PROF(c) ((c)++)
#else
#define OPL3_PROF(c) ((void)0)
#endif
...
OPL3_HOT static void OPL3_EnvelopeUpdateKSL(opl3_slot *slot)
{
...
OPL3_HOT static void OPL3_EnvelopeUpdateRate(opl3_slot *slot)
{
    uint8_t ii;
    ...
    for (ii = 0; ii < 4; ii++)
    {
        ...
        slot->eg_rate_word[ii] = (uint32_t)rate_hi
                               | ((uint32_t)(rate & 0x03) << 8)
                               | ((slot->eg_rates[ii] != 0) ? (1u << 16) : 0u);
    }
}
...
OPL3_HOT static void OPL3_EnvelopeCalc(opl3_slot *slot)
{
    ...
    reset = (slot->key && slot->eg_gen == envelope_gen_num_release);
    {
        ...
        uint8_t idx = reset ? 0 : slot->eg_gen;
        uint32_t rw = slot->eg_rate_word[idx];
        rate_hi = (uint8_t)(rw & 0x0f);
        rate_lo = (uint8_t)((rw >> 8) & 0x03);
...
static OPL3_FORCE_INLINE void OPL3_SlotGenerate(opl3_slot *slot)
{
...
static OPL3_FORCE_INLINE void OPL3_SlotGenerateSilent(opl3_slot *slot)
{
    uint16_t phase = slot->pg_phase_out + *slot->mod;
    uint16_t wf_data = logsin_wf[slot->reg_wf][phase & 0x3ff];
...
OPL3_HOT void OPL3_Generate(opl3_chip *chip, int16_t *buf)
{
    int16_t samples[4];
...
OPL3_HOT void OPL3_WriteReg(opl3_chip *chip, uint16_t reg, uint8_t v)
{
...
OPL3_HOT void OPL3_WriteRegBuffered(opl3_chip *chip, uint16_t reg, uint8_t v)
{
